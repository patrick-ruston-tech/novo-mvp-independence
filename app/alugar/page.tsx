...
export default function AlugarPage() {
  const filteredProperties = properties.filter(p => p.finalidade === 'locacao');

  // Contagem por bairro
  const bairroCounts = bairros.map(b => ({
    ...b,
    count: properties.filter(p => p.finalidade === 'locacao' && p.bairro === b.slug).length
  })).filter(b => b.count > 0);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      ...
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-heading font-bold text-black">
          Imóveis para alugar em São José dos Campos
        </h1>
