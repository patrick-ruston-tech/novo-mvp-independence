...
export default async function ComprarBairroPage({ params }: { params: Promise<{ bairro: string }> }) {
  const resolvedParams = await params;
  const bairroInfo = bairros.find(b => b.slug === resolvedParams.bairro);
  const filteredProperties = properties.filter(p => p.finalidade === 'venda' && p.bairro === resolvedParams.bairro);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-heading font-bold text-black">
          Apartamentos e casas à venda no {bairroInfo.nome}
        </h1>
